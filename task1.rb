...
def parse_product(product_url)
  ...
  (0...product_weight_variation.length).each do |variation_counter|
    ...
    puts product_name, product_img,  product_weight_variation[variation_counter].text.to_s, price_per_weight[variation_counter].text.to_s
...
def write_to_file(file_name, data_to_write, product_name)
  ...
  puts "-----product #{product_name} is written-----\n\n"
end
